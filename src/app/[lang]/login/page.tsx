...
import { getDictionary } from '@/lib/getDictionary'; // For client-side fallback/initial dict
...
export default function LoginPage({ params: { lang } }: { params: { lang: Locale } }) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    await signInWithGoogle(lang); // Pass current language
    setIsGoogleLoading(false);
  };

  const handleGitHubSignIn = async () => {
    setIsGitHubLoading(true);
    await signInWithGitHub(lang); // Pass current language
      ...
      <div className="w-full max-w-md space-y-8">
        <Card className="shadow-xl rounded-lg">
          <CardHeader className="text-center">
            <CardTitle className="text-3xl font-bold tracking-tight text-primary">{dictionary.loginWelcome}</CardTitle>
          ...
          <CardContent className="p-6 sm:p-8">
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-xs uppercase">
                  <span className="bg-card px-2 text-muted-foreground">
                    {dictionary.loginOrContinueWith}
                  </span>
                </div>
              </div>
            ...
            <p className="mt-8 text-center text-sm text-muted-foreground">
              {dictionary.loginNoAccount}{' '}
              <Link href={localizedPath('/signup')} className="font-medium text-accent hover:text-accent/90">
                {dictionary.loginSignUpHere}
              </Link>
            </p>
          </CardContent>
        </Card>
